...
<div class="con">
    <div class="conTop">
        <form action="<%=request.getContextPath()%>/fruit/search?id=user&kind=fruit" method="post">
            ...
            <c:forEach items="${fruitList}" var="F">
                <c:if test="${F.getNum()!=null&&F.getNum()>0}">
                        <tr>
                            ...
                            <td><a href="<%=request.getContextPath()%>/cart/add?name=${F.getName()}&kind=fruit&id=user"><img src="../images/add.png"></a></td>
                        </tr>
                </c:if>
            </c:forEach>
        </table>
            ...
            <c:forEach items="${cartList}" var="C">
                <c:if test="${C.getNum()!=null&&C.getNum()>0}">
                    <tr>
                        ...
                        <td>${C.getPrice()}</td>
                        <td>${C.getNum()}</td>
                        <td><a href="<%=request.getContextPath()%>/cart/delete?name=${C.getName()}&kind=fruit&id=user"><img src="../images/jian.png"></a></td>
                    </tr>
                </c:if>
            </c:forEach>
